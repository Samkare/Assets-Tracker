...
// Vendor master data (suppliers) — create/edit/delete are Admin-only; everyone can read.
export const suppliersRouter = Router();
suppliersRouter.get("/", requireAuth, asyncHandler((req, res) => res.json(svc.listSuppliers())));
suppliersRouter.post("/", requireRole("Admin"), asyncHandler((req, res) => res.status(201).json(svc.createSupplier(supplierInputSchema.parse(req.body)))));
suppliersRouter.put("/:id", requireRole("Admin"), asyncHandler((req, res) => res.json(svc.updateSupplier(req.params.id, supplierInputSchema.partial().parse(req.body)))));
