...
The seed prints the **bootstrap admin** (default `santosh@tasksource.net`). All seeded accounts
start with a temporary password and are forced to set a new one on first login.

| User | Role | Temp password |
|------|------|---------------|
| santosh@tasksource.net (bootstrap) | Admin | from `BOOTSTRAP_ADMIN_PASSWORD` |
